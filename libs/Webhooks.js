...
function buildUrl(options) {
  ...
  let _wbUrl =
    ...
    String(bot.id) +
    "/new-webhook?&command=" +
    encodeURIComponent(options.command);

  if (options.api_key) {
    _wbUrl += "&api_key=" + options.apiKey;
  }
  if (options.public_user_token) {
    _wbUrl += "&public_user_token=" + options.public_user_token;
  }
  if (options.user_id) {
    _wbUrl += "&user_id=" + options.user_id;
  }
  if (options.content) {
    _wbUrl += "&content=" + encodeURIComponent(options.content);
  }
  if (options.redirect_to) {
    _wbUrl += "&redirect_to=" + encodeURIComponent(options.redirect_to);
  }
...
function getGlobalUrl(options) {
  if (!options) {
    throw "WebhooksLib: need pass options for function getGlobalUrl";
  }
...
function getUrlFor(options) {
  ...
  if (!options) {
    throw "WebhooksLib: need pass options for function getUrlFor";
  }
