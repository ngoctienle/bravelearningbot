...
let growthResource = function (resource) {
  return {
    ...
    willCompletedAfter: function () {
      return (
        this.info().interval - (this.progress() / 100) * this.info().interval
      );
    },
    ...
    _getTotalIterationsWithLimit(growth) {
      ...
      if (!growth.max_iterations_count) {
  ...
  return {
    ...
    verifyNumber: function (value) {
      if (!this.isNumber(value)) {
        ...
        throw (
          "ResLib: value must be number only. It is not number: " +
          typeof value +
          " " +
          evalue
        );
    ...
    anywayTakeFromAndTransferTo: function (
      fromResource,
      toResource,
      res_amount
    ) {
    ...
    anywayTakeFromAndTransferToDifferent: function (
      fromResource,
      toResource,
      remove_amount,
      add_amount
    ...
    takeFromAndTransferToDifferent: function (
      fromResource,
      toResource,
      remove_amount,
      add_amount
      ...
      return this.anywayTakeFromAndTransferToDifferent(
        fromResource,
        toResource,
        remove_amount,
        add_amount
    ...
    exchangeTo: function (anotherResource, options) {
      return this.takeFromAndTransferToDifferent(
        this,
        anotherResource,
        options.remove_amount,
        options.add_amount
      );
    },
